...
const ReservationCard: React.FC<UserInfoProps> = (props) => {
  ...
  return (
      <Box mt='4' p='4' bg='#D2691E' borderWidth='1px' borderColor='#800000' borderRadius=''>
        <VStack>
        <Badge mt='2' fontFamily='Agency FB' borderRadius='' px='65' bg='maroon' color='white' fontSize='3xl' width='100%'>
            <Center>{props.hotel.name}</Center>
        </Badge>

        <Box>
            <Text fontFamily='Courier New' color='#800000'>{props.hotel.fullAddress}</Text>
        </Box>
        <Box px='2' borderWidth='1px' borderColor='#800000' borderRadius='' width='100%'>
            <Center><Text fontFamily='Courier New'>ЗАЕЗД: {props.startDate}</Text></Center>
        </Box>
        <Box px='2' borderWidth='1px' borderColor='#800000' borderRadius='' width='100%'>
            <Center><Text fontFamily='Courier New'>ВЫЕЗД: {props.endDate}</Text></Center>
        </Box>
        { status === 'Оплачено' && 
        <Box>
            <Text fontFamily='Courier New'>СТАТУС: { status }</Text>
        </Box> }
        { status === 'Отменено' && 
        <Box>
            <Text fontFamily='Courier New'>СТАТУС: { status }</Text>
        ...
        { (status === 'Оплачено') && 
            <Center>
            <Button bg='maroon' color='white' fontFamily='Courier New' type="submit" onClick={event => submit(event)}>
